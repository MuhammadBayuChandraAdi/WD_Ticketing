...
@php
    ...
    // Safe image URL: use external URL if provided, otherwise use asset (assets path)
    $imageUrl = $image
        ? (filter_var($image, FILTER_VALIDATE_URL)
            ? $image
            : asset('assets/images/' . $image))
        : asset('assets/images/Konser.jpeg');

@endphp

<a href="{{ $href ?? '#' }}" class="block">
    <div class="card bg-base-100 h-96 shadow-sm hover:shadow-md transition-shadow duration-300">
        <div class="h-48 overflow-hidden bg-gray-100 rounded-t-lg">
